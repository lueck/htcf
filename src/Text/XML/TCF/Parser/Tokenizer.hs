...
tokenize cfg tcf = tokenize' tcf
  where
    ...
    tokenize' ((TcfText t1 tOffset1 sOffset1) : (TcfText (t2:t2s) tOffset2 sOffset2) : xs)
    ...
    tokenize' ((TcfText t1 tOffset1 sOffset1):(TcfText t2 _ _):(TcfLineBreak):xs)
    ...
    tokenize' ((TcfText (t:ts) tOffset sOffset):xs)
      -- Drop heading space
      | isSpace t
      = tokenize' ((TcfText
                    (drop spaces ts)
      ...
      | otherwise
      = (Token (t:(take letters ts)) tOffset sOffset)
        : (tokenize'
           ((TcfText
             (drop letters ts)
