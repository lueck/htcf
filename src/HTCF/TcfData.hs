...
instance Csv.ToRecord (PostgresRange TcfData) where
  toRecord (PostgresRange (TcfTokenData tok tid tStart tEnd sStart sEnd ptag tset lem sid fTok fLem))
...
fillTcfData
  (TcfTokenData t1 tid1 ts1 te1 ss1 se1 pt1 set1 l1 sid1 ft1 fl1)
  (TcfTokenData t2 tid2 ts2 te2 ss2 se2 pt2 set2 l2 sid2 ft2 fl2) =
  TcfTokenData
...
mkIdDataTuple d@(TcfTokenData _ tid _ _ _ _ _ _ _ _ _ _) = (fromMaybe 0 tid, d)
...
-- collected in an integer mapping and is returned.
--
...
collectTokenData :: TcfLayers -> (IntMap.IntMap TcfData)
